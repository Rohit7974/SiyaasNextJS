...
const ReelProducts = () => {
  ...
  async function fetchReelProducts() {
    try {
      // Get selected reel product IDs
      const reelIds = JSON.parse(
        localStorage.getItem("siyaas_reel_products") || "[]",
      );

      console.log("Reel IDs:", reelIds);

      // Always fetch products, even if localStorage is empty
      ...
      if (reelIds.length > 0) {
        ...
        reelProducts = reelIds
          .map((id) => allProducts.find((p) => String(p._id) === String(id)))
          .filter(Boolean);
      } else {
        // Fallback: show first 8 products if no reel IDs selected
        reelProducts = allProducts.slice(0, 8);
      }

      console.log("Final Reel Products:", reelProducts);
  ...
  return (
    <div className="relative w-full py-10 px-4">
      ...
      <button
        onClick={scrollLeft}
        className="hidden cursor-pointer md:flex absolute left-2 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/10 hover:bg-black/20 backdrop-blur-xl items-center justify-center shadow-lg"
      >
        ‹
      </button>
      <button
        onClick={scrollRight}
        className="hidden cursor-pointer md:flex absolute right-2 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/10 hover:bg-black/20 backdrop-blur-xl items-center justify-center shadow-lg"
      >
        ›
      </button>
