  ...
  async function fetchFeaturedProducts() {
    try {
      const featuredIds = JSON.parse(
        localStorage.getItem("siyaas_featured_products") || "[]",
      );

      const candlesRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/products?category=candles`,
      );
      const diffusersRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/products?category=diffusers`,
      );
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center mt-10 md:mt-12">
          <button
            ...
            className="bg-[#8B5A2B] cursor-pointer text-white px-6 md:px-8 py-2 md:py-3 rounded-md hover:bg-gray-800 transition text-sm md:text-base"
          >
            View all
          </button>
        </div>
